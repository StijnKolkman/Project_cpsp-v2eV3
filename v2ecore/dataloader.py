...
class FramesDirectory(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for image in [image_1, image_2]:
            # Open image using pil.
            image = Image.fromarray(image)
            image = image.resize(self.dim, Image.Resampling.LANCZOS) #IMPORTANT: CHANGED Image.ANTIALIAS TO Image.Resampling.LANCZOS FOR COMPATABILITY ISSUES WITH PILLOW 9.0.0
            # Apply transformation if specified.
            if self.transform is not None:
                image = self.transform(image)
            sample.append(image)
