...
def lin_log(x, threshold=20): # This function is ready for batch processing
    ...
    :param x: torch.Tensor
        Input tensor with shape (batch, height, width), 
        values in the range 0-255 (assumed to be 8-bit).
    :param threshold: float
        Threshold for transition from linear to log mapping.

    Returns: torch.Tensor
        The transformed tensor with the same shape as input x.
    """
    # Ensure the input is in float64 for precision
    if x.dtype is not torch.float64:
        x = x.double()

    # Compute the linear scaling factor
    f = (1. / threshold) * math.log(threshold)

    # Apply the linear or logarithmic mapping
    # Use torch.where for batched input
...
def compute_event_map(diff_frame, pos_thres, neg_thres):
    ...
    Args:
        diff_frame:  the input difference frame between stored log intensity and current frame log intensity [batch_size, height, width]
        pos_thres:   ON threshold value (scalar)
        neg_thres:   OFF threshold value (scalar)
